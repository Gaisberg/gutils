...
function LibDraw.GroundCircle(x, y, z, size)
	...
	for v=0, full_circle, small_circle_step do
		fx, fy, fz = _G.TraceLine(  (x+cos(v)*size), (y+sin(v)*size), z+100, (x+cos(v)*size), (y+sin(v)*size), z-100, flags )
		if fx == nil then
...
function LibDraw.Texture(config, x, y, z, alphaA)
	...
	if not scale then
		local cx, cy, cz = _G.GetCameraPosition()
...
function LibDraw.DrawHelper()
	local playerX, playerY, playerZ = _G.ObjectPosition("player")
...
function LibDraw.Camera()
	local fX, fY, fZ = _G.ObjectPosition("player")
	local sX, sY, sZ = _G.GetCameraPosition()
